...
function get_one_valid_opponent_move() {
    ...
    const src = rotateCoord(rotated_coord);
                ...
                if (Math.random() < candidates.length / (candidates.length + candidates_inf.length)) {
                    ...
                    const obj = {
                        ...
                        finalResult: new Promise((resolve, reject) => {
                            const filteredList = filterInOneDirectionTillCiurlLimit(candidates_inf, step, planned_dest, stepping_ciurl).filter((dest) => canGetOccupiedBy(Side.Downward, dest, {
                                color: rotated_piece.color,
                                prof: rotated_piece.prof,
                                side: Side.Downward,
                            }, GAME_STATE.f.currentBoard, GAME_STATE.tam_itself_is_tam_hue));
 ...
 */
function takeTheUpwardPieceAndCheckHand(destPiece) {
    const flipped = (() => {
        if (destPiece === "Tam2") {
...
async function animateOpponentSrcDst_(src, dst, water_entry_ciurl) {
    ...
    if (destPiece !== null) {
        ...
        await animateNode(destNode, total_duration, indToHo1Zuo1OfDownward(GAME_STATE.f.hop1zuo1OfDownward.length), /* not yet pushed into GAME_STATE.f.hop1zuo1OfDownward */ coordToPieceXY([dest_i, dest_j]), "50", 180);
        if (water_entry_ciurl) {
            await animateWaterEntryLogo();
            displayCiurl(water_entry_ciurl, Side.Downward);
            await new Promise((resolve) => setTimeout(resolve, 500 * 0.8093));
