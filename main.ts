...
function takeTheDownwardPiece(destPiece: Piece){
    if (destPiece === "Tam2") {
        ...
        if (new_state.score === old_state.score) {
            ...
            setTimeout(
                () => alert(`new hand detected; hands: ${new_state.hands}, score: ${new_state.score}`),
                1000 * 0.8093
            );
        }
...
function updateField(message: NormalMove) {
    if (message.type === "NonTamMove") {
        ...
        const piece: Piece | null = GAME_STATE.f.currentBoard[firstDest_i][firstDest_j];
        if (piece === null) {
...
function display_guides(coord: Coord, piece: "Tam2" | NonTam2PieceUpward, parent: HTMLElement, list: Coord[]) {
    for (let ind = 0; ind < list.length; ind++) {
...
function drawScoreDisplay(hands: Hand[]) {
    ...
    function drawHandAndScore(hand: Hand, left: number){
