...
const initialize_funcs = (TOTAL_PROB_NUM) => {
    ...
    function submit_ans(salt) {
        ...
        const f = (hand_id, hand, hand_flash) => {
            const normal_node = document.getElementById(hand_id + "_" + salt);
            normal_node.disabled = true;
            const flash_node = document.getElementById(hand_id + "_flash_" + salt);
            flash_node.disabled = true;
            const no_node = document.getElementById(hand_id + "_not_" + salt);
        ...
        f("cavalry", "馬弓兵", "同色馬弓兵");
        f("attack", "行行", "同色行行");
        const king_node = document.getElementById("king_" + salt);
        king_node.disabled = true;
        const no_king_node = document.getElementById("king_not_" + salt);
